...
async def show_shop(client, message):
    ...
    if not characters:
        await message.reply("🌌 The Cosmic Bazaar is empty! No slaves are available yet.")
        return
    ...
    caption_message = (
        ...
        f"**slave:** {character['name']}\n"
        f"**Anime:** {character['anime']}\n"
        f"**Rarity:** {character['rarity']}\n"
        f"**Cost:** {character['price']} Star Coins\n"
        f"**ID:** {character['id']}\n\n"
        f"✨ Summon this epic slave to join your collection! ✨"
    )

    keyboard = [
        [InlineKeyboardButton("Claim Slave!", callback_data=f"buy_{current_index}"),
         InlineKeyboardButton("Next Slave", callback_data="next")]
    ]
...
async def buy_character(client, callback_query):
    ...
    if current_index >= len(characters):
        await callback_query.answer("🚫 This slave has vanished from the Cosmic Bazaar!", show_alert=True)
        return
    ...
    if not user:
        await callback_query.answer("🚫 Traveler, you must register in the Cosmos to claim slave!", show_alert=True)
        return
    ...
    await callback_query.answer("🎉 Slave claimed! Your new legend joins the Cosmos!")
...
async def next_item(client, callback_query):
    ...
    if not characters:
        await callback_query.answer("🌌 No more Slaves remain in the Cosmic Bazaar!", show_alert=True)
        return
    ...
    keyboard = [
        [InlineKeyboardButton("Claim Slave!", callback_data=f"buy_{next_index}"),
         InlineKeyboardButton("Next Slave", callback_data="next")]
    ]
...
async def add_to_shop(client, message):
    ...
    if len(args) != 2:
        await message.reply("🌌 Usage: /addshop <id> <price> to add a slave to the Cosmic Bazaar!")
        return
    ...
    try:
        ...
        await message.reply("🚫 The price must be a valid number of Coins!")
        return

    character = await collection.find_one({"id": character_id})
    if not character:
        await message.reply("🚫 This slave doesn't exist in the Cosmos!")
        return
...
async def remove_from_shop(client, message):
    ...
    if len(args) != 1:
        await message.reply("🌌 Usage: /removeshop <id> to remove a slave from the Cosmic Bazaar!")
        return
    ...
    if not character:
        await message.reply("🚫 This slave is not in the Cosmic Bazaar!")
        return
