# LCatch/commands/search_characters.py
from LCatch import app, collection, rarity_map2 as rarity_map
...
async def handle_search(client, message, query=None, page=1, is_callback=False):
    try:
        if not query:
            ...
            if len(args) < 2:
                await message.reply_text("Please provide a character name. Usage: /q {character name}")
                return
...
@app.on_message(filters.command("1"))
