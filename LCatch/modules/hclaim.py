...
from LCatch import user_collection, collection, user_nguess_progress, user_guess_progress, SUPPORT_CHAT_ID as chat
...
async def get_unique_characters(user_id, target_rarities=['⚪️ Common', '🟠 Nobel']):
    try:
...
@bot.on_message(filters.command(["hclaim", "claim", "sclaim"]))
async def mclaim(_, message: t.Message):
    ...
    try:
        ...
        if str(message.chat.id) != chat:
            join_button = InlineKeyboardMarkup([[InlineKeyboardButton("Join Here", url="https://t.me/+Qq_DJrLZyOAxOWE1")]])
            return await message.reply_text(
                "🔔 Jᴏɪɴ ʟᴜsᴛ sᴜᴘᴘᴏʀᴛ ᴛᴏ ᴄʟᴀɪᴍ ʏᴏᴜʀ ᴅᴀɪʟʏ sʟᴀᴠᴇ 🔔",
